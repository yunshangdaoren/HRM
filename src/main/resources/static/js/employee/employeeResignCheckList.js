//定义全局部门查询要跳转的地址
var href = "http://localhost:8080/employeeResign/toEmployeeResignCheckList.do?pageNum=";
//当前页码
...
$("#span-jumPageNum").click(function(){
	...
	if($.isNumeric(pageNum)){
		...
		if(pageNum > totalPage || pageNum <= 0){
			...
			window.location.href = "/employeeResign/toEmployeeResignCheckList.do?pageNum="+pageNum+"&"+$("#form-queryDept").serialize();
		}
	}else{
		alert("请输入正确的页码！")
	}
...
//审批点击事件
$(".a_checkEmployeeResign").click(function(){
	...
	if(value == true){
		...
		//发送Ajax请求
		$.ajax({
			url:"/employeeResign/employeeResignCheck.do?resignId="+resignId,
			dataType:"json",
			type:"post",
