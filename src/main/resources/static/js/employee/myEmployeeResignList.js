//定义全局部门查询要跳转的地址
var href = "http://localhost:8080/employeeResign/toMyEmployeeResignList.do?pageNum=";
//当前页码
...
$("#span-jumPageNum").click(function(){
	...
	if($.isNumeric(pageNum)){
		...
		if(pageNum > totalPage || pageNum <= 0){
			...
			window.location.href = "/employee/toMyEmployeeResignList.do?pageNum="+pageNum+"&"+$("#form-queryDept").serialize();
		}
	}else{
		alert("请输入正确的页码！")
	}
...
$(".a_detailEmployee").click(function(){
	var empJobId = $(this).parent().parent().children().second().text();
	alert(empJobId);
