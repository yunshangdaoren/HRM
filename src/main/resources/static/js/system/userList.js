//定义全局部门查询要跳转的地址
var href = "http://localhost:8080//user/toUserList.do?pageNum=";
//当前页码
...
$("#span-jumPageNum").click(function(){
	...
	if($.isNumeric(pageNum)){
		...
		if(pageNum > totalPage || pageNum <= 0){
			...
			window.location.href = "/user/toUserList.do?pageNum="+pageNum+"&"+$("#form-queryDept").serialize();
		}
	}else{
		alert("请输入正确的页码！")
	}
...
$(".a_updateUser").click(function(){
	//获取到账户
	var userAccount = $(this).parent().parent().children().first().text();
