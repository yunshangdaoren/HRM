//定义全局部门查询要跳转的地址
var href = "http://localhost:8080/contract/toCheckEntryContractList.do?pageNum=";
//当前页码
...
$("#span-jumPageNum").click(function(){
	...
	if($.isNumeric(pageNum)){
		...
		if(pageNum > totalPage || pageNum <= 0){
			...
			window.location.href = "/contract/toCheckEntryContractList.do?pageNum="+pageNum+"&"+$("#form-queryDept").serialize();
		}
	}else{
		alert("请输入正确的页码！")
	}
...
//审批待入职合同
$(".a_checkEntryContract").click(function(){
	...
	if(value == true){
		...
		//发送Ajax请求
		$.ajax({
			url:"/contract/checkEntryContract.do?conId="+conId,
			dataType:"json",
			type:"post",
			success:function(result){
				if(result.code==200){
