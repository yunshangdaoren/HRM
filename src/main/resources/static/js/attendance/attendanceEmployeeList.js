//定义全局部门查询要跳转的地址
var href = "http://localhost:8080/attendanceEmployee/toAttendanceEmployeeList.do?pageNum=";
//当前页码
...
$("#span-jumPageNum").click(function(){
	...
	if($.isNumeric(pageNum)){
		...
		if(pageNum > totalPage || pageNum <= 0){
			...
			window.location.href = "/attendanceEmployee/toAttendanceEmployeeList.do?pageNum="+pageNum+"&"+$("#form-queryDept").serialize();
		}
	}else{
		alert("请输入正确的页码！")
	}
...
//跳转到职工详情页面
$(".a_employeeDetail").click(function(){
	var empJobId = $(this).parent().parent().children().eq(1).text();
